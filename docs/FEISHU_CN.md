...
- **私聊 + 群聊** — 支持单聊和群聊。
- **群聊安全默认值** — 默认要求 @机器人。
- **文本 + 入站图片** — 支持文本、富文本、语音转文字，以及可交给 Codex
  查看和理解的图片资源。
...
对 `image` 消息和飞书富文本（`post`）中嵌入的图片，clawdex 会把图片资源下载
到临时目录，并把本地图片路径传给 Codex。非图片附件目前会以文本占位符表示。
...
- 如果图片消息只显示为占位符，检查应用是否已开通消息资源读取权限并重新发布。
